...
<div class="p25">
    ...
    if (function_exists('get_promotions_en_cours')) {
        $promotions_en_cours = get_promotions_en_cours();
        if (!empty($promotions_en_cours)) {
            echo '<div class="promotions-en-cours">';
            
            echo '<ul class="list-unstyled">';
            foreach ($promotions_en_cours as $promo) {
                
                if (isset($promo['statut']) && strtolower($promo['statut']) === 'active') {
                    echo '<li>';
                    echo '<strong>' . htmlspecialchars($promo['nom']) . '</strong>';
                    
                    if (isset($promo['dateDebut']) && isset($promo['dateFin'])) {
                        echo ' <small>(' . htmlspecialchars($promo['dateDebut']) . ' - ' . htmlspecialchars($promo['dateFin']) . ')</small>';
                    }
                    echo '</li>';
                }
            }
            ...
            echo 'Aucune promotion en cours actuellement';
        }
    } else {
    ...
    ?>
        ...
        <nav class="menu">
            ...
            <a href="index.php?page=apprenant"><i class="fas fa-users"></i> Apprenants<label for="menu-toggle" class="menu-closer"></label></a>
